...
namespace
{
    ...
    void ReachAwaitRoute(RemoteRouteProtocol& protocol, std::int64_t startMs = 100)
    {
        ...
        Require(protocol.Tick(selection, startMs + 30).command ==
            ...
            "exact cursorless selection did not request stock Set Course");
    ...
    void TestGalaxyGateRequiresTheSystemFormRatherThanTheDisplayedBody()
    {
        ...
        Require(!protocol.Tick(bodyIdentity, 110).failed &&
                protocol.CurrentPhase() == RemoteRouteProtocol::Phase::AwaitGalaxy,
            "a displayed body FormID was mistaken for the destination STDT root");

        Require(!protocol.Tick(GalaxyInput(), 120).failed &&
                protocol.CurrentPhase() == RemoteRouteProtocol::Phase::EstablishSelection,
            "the exact destination STDT did not complete the galaxy gate");
    }
