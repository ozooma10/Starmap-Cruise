...
TransitionResult NavigationRuntime::ObserveRemoteArrival(RemoteArrivalObservation observation)
{
    ...
    if (!observation.mapClosed || !observation.loadingMenuClosed ||
        !observation.completedPlayerJump || !observation.settledFlight ||
        !observation.flying || !observation.freshHudPublication ||
        observation.currentSystem != operation.destination.system) {
        return {};
    }

    const auto finalCount = static_cast<std::size_t>(std::count(
        observation.courseRows.begin(), observation.courseRows.end(),
        operation.destination.courseId));
    if (finalCount > 1) {
        Reset();
        return {.handled = true};
    }
    ...
    if (finalCount == 0) {
        ...
        for (const auto row : observation.courseRows) {
            const auto found = std::find(operation.destination.remotePlan.allowedWaypointIds.begin(),
                operation.destination.remotePlan.allowedWaypointIds.end(), row);
...
TransitionResult NavigationRuntime::CruiseChanged(bool active)
{
    ...
    if (!active && !m_state.remoteOperation &&
        (m_state.phase == NavigationPhase::AwaitingCourseLock || m_state.phase == NavigationPhase::CourseLocked)) {
        m_state.phase = NavigationPhase::Marked;
        return {.handled = true};
    }
    return {};
...
bool NavigationRuntime::RecoverFromEffectFailure(const Effect& effect)
{
    ...
    if (const auto* press = std::get_if<PressCruise>(&effect)) {
        if (press->operationId != 0) {
            if (!m_state.remoteOperation || m_state.remoteOperation->id != press->operationId ||
                m_state.phase != NavigationPhase::PreparingRemoteTarget) {
                return false;
            }
            Reset();
    ...
    if (!request || m_state.phase != NavigationPhase::AwaitingCourseLock ||
        !m_state.destination || request->courseId != m_state.destination->courseId) {
        return false;
    }
...
{
    return m_state.remoteOperation && m_state.remoteOperation->id == operationId &&
        m_state.remoteOperation->source == source;
}
