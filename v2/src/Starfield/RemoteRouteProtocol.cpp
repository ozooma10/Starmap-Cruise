...
RemoteRouteProtocol::Decision RemoteRouteProtocol::Tick(const TickInput& input, std::int64_t nowMs)
{
    ...
    const bool newEndpoint = input.routeReadable && input.routeEndpoint != 0 && input.routeEndpoint != m_preexistingEndpoint;
    ...
    const bool ready = newEndpoint && input.endpointIdentity == EndpointIdentity::ExactDestination && input.executeGateResolved && input.executeReady;
    if (!ready) {
        m_readySinceMs = -1;
...
RemoteRouteProtocol::CloseResult RemoteRouteProtocol::MapClosed(bool sourceMatches, std::uint64_t executeEventCount)
{
    ...
    if (!sourceMatches || m_phase != Phase::AwaitExecuteClose ||
        ...
        Fail(m_phase == Phase::AwaitExecuteClose ? "Starmap closed without the correlated native Execute event" : "Starmap closed before the remote route committed");
