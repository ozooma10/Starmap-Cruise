...
namespace
{
    ...
    void RequireNoSourceCalls(
        const FakeBodyResolutionSource& source,
    ...
    void TestInvalidDossierDoesNotQuerySource()
    {
        ...
        Require(result.dossierId == 0,
            "invalid dossier produced a nonzero identity");
        Require(!result.dossierIsLiveBody,
            "invalid dossier was reported as live");
        Require(!result.bodyIndexReady,
            "invalid dossier reported index readiness");
        Require(!result.indexedBody,
            "invalid dossier produced an indexed body");
        RequireNoSourceCalls(source,
            "invalid dossier queried the resolution source");
    }
    ...
    void TestUnsupportedDossierDoesNotQuerySource()
    {
        ...
        Require(result.dossierId == JemisonId,
            "unsupported dossier lost its identity");
        Require(!result.dossierIsLiveBody,
            "unsupported dossier was reported as live");
        Require(!result.bodyIndexReady,
            "unsupported dossier reported index readiness");
        Require(!result.indexedBody,
            "unsupported dossier produced an indexed body");
        RequireNoSourceCalls(source,
            "unsupported dossier queried the resolution source");
    }
    ...
    void TestNonLiveBodyDoesNotQueryIndexEntry()
    {
        ...
        Require(result.dossierId == JemisonId,
            "non-live result lost the dossier identity");
        Require(!result.dossierIsLiveBody,
            "non-live body was reported as live");
        Require(result.bodyIndexReady,
            "independent index readiness was not captured");
        Require(!result.indexedBody,
            "non-live body produced an indexed observation");
        Require(source.liveBodyCalls == 1 &&
                source.lastLiveBodyId == JemisonId,
            "live-form lookup did not receive the dossier identity");
        Require(source.indexReadyCalls == 1,
            "index readiness was not captured exactly once");
        Require(source.findBodyCalls == 0,
            "non-live body queried the index entry");
    }
    ...
    void TestLoadingIndexDoesNotQueryEntry()
    {
        ...
        Require(result.dossierIsLiveBody,
            "live body was not reported as live");
        Require(!result.bodyIndexReady,
            "loading index was reported as ready");
        Require(!result.indexedBody,
            "loading index produced an indexed observation");
        Require(source.findBodyCalls == 0,
            "loading index was queried for an entry");
    }
    ...
    void TestReadyIndexCanReportMissingBody()
    {
        ...
        Require(result.dossierIsLiveBody,
            "ready missing body lost live-form evidence");
        Require(result.bodyIndexReady,
            "ready missing body lost index readiness");
        Require(!result.indexedBody,
            "missing body produced an indexed observation");
        Require(source.findBodyCalls == 1 &&
                source.lastFindBodyId == JemisonId,
            "ready index was not queried with the dossier identity");
    }
    ...
    void TestValidIndexedMoonProducesCompleteResolution()
    {
        ...
        Require(result.bodyIndexReady,
            "complete resolution lost index readiness");
        Require(result.indexedBody.has_value(),
            "valid indexed body produced no observation");
        Require(result.indexedBody->id == JemisonId,
            "complete resolution retained the wrong body identity");
        Require(result.indexedBody->systemId == AlphaCentauriId,
            "complete resolution retained the wrong system identity");
        Require(source.findBodyCalls == 1 &&
                source.lastFindBodyId == JemisonId,
            "complete resolution queried the wrong index identity");
    }
