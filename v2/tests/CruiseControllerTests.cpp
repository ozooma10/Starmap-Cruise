...
namespace
{
    ...
    void OpenMap(::CruiseController& controller, bool cruiseWasActive = false)
    {
        controller.OnMapMovieCreated(CurrentIdentity.generation);

        Require(
            controller.OnMapOpened({
        ...
        Require(controller.OnMapViewChanged(CurrentIdentity, ::MapView::System), "controller rejected system view");

        Require(
            controller.OnMarkersChanged(
                CurrentIdentity,
                {
                    .highlightedCount = 1,
                    .highlighted =
        ...
        Require(controller.OnDossierChanged(CurrentIdentity, JemisonDossier()), "controller rejected dossier observation");
    }
    ...
    void TestFullTapFlow()
    {
        ...
        Require(controller.CurrentNavigationState().phase == ::NavigationPhase::ClosingMap, "tap did not wait for the map-close observation");
        ...
        Require(closed.completedCount == 0, "plain mark dispatched a command after map close");
        Require(commands.calls.size() == 1, "plain mark issued an unexpected second command");
        Require(controller.CurrentNavigationState().phase == ::NavigationPhase::Marked, "tap did not finish as a retained mark");
    }
    ...
    void TestFullHoldFlow()
    {
        ...
        Require(controller.ActivateMapAction(CurrentIdentity, ::MapActionGesture::HoldCompleted, ReadyEnvironment()).Succeeded(), "completed hold did not close the map");

        Require(controller.OnMapClosed(CurrentIdentity).Succeeded(), "map close did not dispatch Cruise activation");

        Require(controller.OnCruiseChanged(true).Succeeded(), "Cruise activation did not dispatch the course request");
        ...
        Require(locked.completedCount == 0, "course-lock confirmation emitted an unexpected command");
    ...
    void TestStaleEventsDoNotResolveOrDispatch()
    {
        ...
        Require(!activated.handled, "stale action was reported as handled");
        Require(commands.calls.empty(), "stale action dispatched a command");
        Require(controller.CurrentNavigationState().phase == ::NavigationPhase::Idle, "stale events changed navigation state");
    }
    ...
    void TestFailedCloseMapRecoversAutomatically()
    {
        ...
        Require(activated.failedEffect.has_value() && std::get_if<::CloseMap>(&*activated.failedEffect) != nullptr, "failed CloseMap command retained the wrong effect");
        Require(controller.CurrentNavigationState().phase == ::NavigationPhase::Idle, "failed CloseMap command left navigation stuck");
        Require(!controller.CurrentNavigationState().destination, "failed CloseMap command retained an incomplete destination");
    }
    ...
    void TestFailedCruisePressFallsBackToMark()
    {
        ...
        Require(closed.failedEffect.has_value() && std::get_if<::PressCruise>(&*closed.failedEffect) != nullptr, "failed PressCruise command retained the wrong effect");
        Require(controller.CurrentNavigationState().phase == ::NavigationPhase::Marked, "failed PressCruise command did not fall back to a mark");
        Require(controller.CurrentNavigationState().destination.has_value(), "failed PressCruise command discarded the destination");
    }
