...
namespace
{
    ...
    void TestUnhandledTransitionDoesNotDispatch()
    {
        ...
        Require(!result.handled,
            "unhandled transition was reported as handled");
        Require(!result.Succeeded(),
            "unhandled transition was reported as successful");
        Require(result.completedCount == 0,
            "unhandled transition completed an effect");
        Require(!result.failedEffect,
            "unhandled transition reported an execution failure");
        Require(commands.calls.empty(),
            "unhandled transition dispatched a command");
    }
    ...
    void TestHandledTransitionWithoutEffectsSucceeds()
    {
        ...
        Require(result.handled,
            "handled transition lost its handled state");
        Require(result.Succeeded(),
            "handled transition without effects did not succeed");
        Require(result.completedCount == 0,
            "empty transition reported a completed effect");
        Require(!result.failedEffect,
            "empty transition reported an execution failure");
        Require(commands.calls.empty(),
            "empty transition dispatched a command");
    }
    ...
    void TestDispatchesAllEffectsInOrder()
    {
        ...
        Require(result.Succeeded(),
            "successful commands produced a dispatch failure");
        Require(result.completedCount == 3,
            "dispatcher reported the wrong completed count");
        Require(commands.calls.size() == 3,
            "dispatcher issued the wrong number of commands");
        Require(commands.calls[0].command == RecordedCommand::CloseMap,
            "CloseMap was not dispatched first");
        Require(commands.calls[1].command == RecordedCommand::PressCruise,
            "PressCruise was not dispatched second");
        Require(commands.calls[2].command == RecordedCommand::RequestCourse,
            "RequestCourse was not dispatched third");
        Require(commands.calls[2].courseId == 0x12345678,
            "RequestCourse received the wrong course ID");
    }
    ...
    void TestStopsOnFirstFailure()
    {
        ...
        Require(result.handled,
            "failed dispatch lost the transition's handled state");
        Require(!result.Succeeded(),
            "failed command was reported as successful");
        Require(result.completedCount == 1,
            "failed command was counted as completed");
        Require(result.failedEffect.has_value(),
            "failed command did not retain its effect");
        Require(std::get_if<::PressCruise>(&*result.failedEffect) != nullptr,
            "dispatcher retained the wrong failed effect");
        Require(commands.calls.size() == 2,
            "dispatcher continued after the first failed command");
        Require(commands.calls[0].command == RecordedCommand::CloseMap,
            "successful command before failure was not issued");
        Require(commands.calls[1].command == RecordedCommand::PressCruise,
            "expected failing command was not issued");
    }
